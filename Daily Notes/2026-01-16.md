...
延续[[Daily Notes/2026-01-14]]：今天继续练习把矩阵化到（行）阶梯形。  
Continuation of [[Daily Notes/2026-01-14]]: today we kept practicing reducing a matrix to (row) echelon form.

（练习）把每个主元（pivot）圈出来很有帮助。  
(Practice) Circling pivots helps a lot.

对每一行重复以下步骤：  
Repeat the steps for each row:
- 选定一个主元（pivot），并用行倍乘把它化为$1$。  
  Choose a pivot and scale the row so the pivot becomes $1$.
- 消去该主元下方（below pivot）的所有元素，使它们变为$0$。  
  Eliminate all entries below the pivot so they become $0$.

达到阶梯形（echelon form）的判据：  
Criteria for echelon form:
1) 每个主元下面的元素全为$0$。  
   All entries below each pivot are $0$.
2) 越往下的行，其主元位置要在上面行主元的右侧（整体“向下且向右”）。  
   Pivots in lower rows must be to the right of pivots in higher rows (overall “down & right”).

### Worked Example / 例子（化到阶梯形）
从增广矩阵开始：  
...
第1列取主元$1$（第1行第1列），消去下方元素：  
Pivot at $1$ in column 1 (row 1), eliminate below:
...
第2列主元为$1$（第2行第2列），继续消去下方：  
Pivot at $1$ in column 2 (row 2), eliminate below:
...
此时已是阶梯形：主元位置依次向右移动，且每个主元下面全为$0$。  
This is echelon form: pivots move to the right as you go down, and entries below each pivot are all $0$.

## 已整理 / Extracted
- [[Math_Calculus/5.Differential_Equations/differential_equations_basics]]（微分方程基础）。  
  [[Math_Calculus/5.Differential_Equations/differential_equations_basics]] (differential equations basics).
- [[Physics/1.Vectors/vectors_and_scalars]]（标量与矢量）。  
  [[Physics/1.Vectors/vectors_and_scalars]] (scalars and vectors).
- [[Math_Linear-Algebra/gauss_jordan_elimination]]（行变换与阶梯形）。  
  [[Math_Linear-Algebra/gauss_jordan_elimination]] (row operations and echelon form).
