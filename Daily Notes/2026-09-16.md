...
#### 电荷、电场与电荷密度 / Charge, Field, and Charge Density
...
#### 几何量、高斯定律与导体 / Geometry, Gauss's Law, and Conductors
...
#### 电势、能量与运动 / Potential, Energy, and Motion
...
这里的两个电荷量分别是： / The two charge symbols mean:

- **$q$：放在空腔里的带电物体所带的净电荷**，该物体不与导体接触；若有多个带电物体，$q$ 是它们电荷的代数和，不包含空腔壁上的电荷。 / **$q$: the net charge of the charged object placed inside the cavity**, without touching the conductor. For multiple objects, it is the algebraic sum of their charges, excluding charge on the cavity wall.
- **$Q_{\mathrm{inner}}$：导体内表面（空腔壁）上的感应电荷总量**，属于导体本身；下标 inner 指“内表面”。它是整个内表面电荷的代数和，不是某一点的面电荷密度。 / **$Q_{\mathrm{inner}}$: the total induced charge on the conductor's inner surface (the cavity wall)**, belonging to the conductor itself. The subscript inner refers to this surface. It is the algebraic sum over the entire inner surface, not the surface charge density at one point.

包围空腔且位于金属中的高斯面同时包住这两部分电荷，所以包围的净电荷为 $Q_{\mathrm{enc}}=q+Q_{\mathrm{inner}}$；导体外表面的电荷不在这个高斯面内。 / A Gaussian surface within the metal and surrounding the cavity encloses both contributions, so $Q_{\mathrm{enc}}=q+Q_{\mathrm{inner}}$. Charge on the conductor's outer surface lies outside this Gaussian surface.
